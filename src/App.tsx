...
export default function App() {
  ...
  const handleFormSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const formspreeResponse = await fetch("https://formspree.io/f/mdaqkjpv", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          phone: phone.trim(),
          caseType: caseType,
          content: content.trim() || "음주운전 행정구제 상담 희망"
        })
      });
      ...
      const discordPayload = {
        embeds: [
          {
            title: "🚨 새 무료 행정구제 상담 신청",
            color: 0xD4AF37, // Gold color
            fields: [
              { name: "👤 이름", value: name.trim(), inline: true },
              { name: "📞 연락처", value: phone.trim(), inline: true },
              { name: "⚖️ 사건 유형", value: caseType, inline: true },
              { name: "📝 상담 내용", value: content.trim() || "음주운전 행정구제 상담 희망", inline: false }
            ],
      ...
      await fetch("https://discord.com/api/webhooks/1526838764905828414/79h98cQ-7lQWLjE9IzfszmGDPVqFEl0FzQPzcx-A1IXbjvGoTc1uUhj7B41L1i9c-roc", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      // Add to local lead listings for immediate feedback/social proof (masked for privacy)
      const newLead: Lead = {
      ...
      alert("상담 신청 중 오류가 발생했습니다. 잠시 후 다시 시도해주시거나 전화 상담(010-8114-4300)을 이용해 주세요.");
    } finally {
